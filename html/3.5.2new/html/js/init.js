...
var vodurl="http://www.miguvideo.com/gateway/program/v1/cont/content-info/";//没有用到顶部的加密地址，直接申明的变量 新的获取播放详情接口
var vodplayUrl = "http://pc.miguvideo.com/playurl/v3/play/playurl?";
//var historyUrl = "http://www.miguvideo.com/uic-service/playHistory/add?";//旧版的接口注掉
var historyUrl = "http://www.miguvideo.com/gateway/uic/v1/user-play-history/?";
	...
	try{
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4&&xhr.status==200) {
				...
				var data = empInfo.body.data;

				//剧集列表
				name = data.playing.name;//节目名称
				//如果是电影就没有cmId  assetID: "5102237977",epsAssetID: "5102235101"
				                 // mId: "5102235101",cmId: "5102237977",,
				//返回结果结果中如果没有epsAssetID这个字段 表示是单集类节目
				if (typeof(data.epsAssetID) =="undefined" ) {
					cmId = "";
					mId = data.assetID
				}else {
					mId = data.epsAssetID;//媒资ID(剧集壳)
					cmId = data.assetID;//媒资ID(子集)
				}
				prdInfoIds = data.prdPackId;
				var desimg = data.pics.highResolutionV;//电视剧封面图
				imgHstr = data.pics.highResolutionH;
				if(desimg==""){
					desimg = data.pics.highResolutionH;
					if(desimg==""){
						desimg="pic/default.png";
					}
				}			
				document.getElementById("tinfo101").innerHTML=subString(name,22,true);
				document.getElementById("tinfo102").innerHTML=data.updateEP;//剧集更新详情
				...
				if (typeof(data.director) == "undefined"){
					...
					direcotor=data.director
				}
				if (typeof(data.actor) == "undefined"){
					actor=""
				}else {
					actor=data.actor
				}
				if (typeof(data.contentStyle) == "undefined"){
					contentStyle=""
				}else {
				...
				document.getElementById("destag2").innerHTML="导演："+direcotor;//导演
				document.getElementById("destag3").innerHTML="主演："+actor;//主演
				document.getElementById("destag4").innerHTML="类型："+contentStyle;//节目类型
				document.getElementById("destag5").innerHTML="地区："+area;//节目地区
				
				var detail = data.detail;//简介
				cids=data.playing.pID;//当前节目id

				//拼接SubSerial_IDS start
				if (typeof(data.datas) == "undefined"){
					SubSerial_IDS = "";
				}else{
					SubSerial_IDS = data.datas;
					SubSerial_IDS = getSubSerial_IDS_List(SubSerial_IDS);
				}
				//SubSerial_IDS = getSubSerial_IDS_List(SubSerial_IDS);
				//拼接SubSerial_IDS end
				...
				//isPingBi = empInfo[0].isPingBi;//版权 确实是否屏蔽版权字段
				if(nextFlag()){
					nextF=1;
				}
				/**
				...
				var playType = data.programTypeV2;//电视剧 programTypeV2=TV_PLAY，电影programTypeV2=MOVIE；动漫programTypeV2=MANGA;少儿programTypeV2=JUVENILE
				//getRecommendData(contDisplayType,mediaType,mediaShape,playType)
				if(playType=="TV_PLAY"|| playType=="MANGA" || playType =="JUVENILE" ){
					initMenu(1,1);										
					if(playType=="TV_PLAY" && SubSerial_IDS!=""||(playType=="MANGA") || (playType=="JUVENILE") ){
						iszhuiju(1);
					}
					var curNumber = data.playing.pID;//当前播放节目id
					var tvlen = data.datas.length;
					var tvint = parseInt(tvlen/5);
					var tvmore = tvlen%5;
					var tmpCidlist="";
					var tmpCurrentSeries=1;
					for(var m=0;m<tvlen;m++){
						tmpCidlist += data.datas[m].pID+",";
						if(curNumber==data.datas[m].pID)tmpCurrentSeries = m+1;
					}		
					...
					for(var i=0;i<tvint;i++){
						...
						var id1 = data.datas[5*i+1].pID;
						if(curNumber==id1){
							divStr+="<td background='pic/btn_pre2x.png' width='47px' height='47px' id='title4' align='center' >"+(5*i+2)+"</td>";
						}else{
							divStr+="<td background='pic/btn_jj2x.png' width='47px' height='47px' id='title3' align='center' onmouseover=\"this.style.backgroundImage='url(pic/btn_hover2x.png)'\" onmouseout=\"this.style.backgroundImage='url(pic/btn_jj2x.png)'\" onclick='clicktv("+data.datas[5*i+1].pID+");' >"+(5*i+2)+"</td>";
						}
						var id2 = data.datas[5*i+2].pID;
						if(curNumber==id2){
							divStr+="<td background='pic/btn_pre2x.png' width='47px' height='47px' id='title4' align='center' >"+(5*i+3)+"</td>";
						}else{
							divStr+="<td background='pic/btn_jj2x.png' width='47px' height='47px' id='title3' align='center' onmouseover=\"this.style.backgroundImage='url(pic/btn_hover2x.png)'\" onmouseout=\"this.style.backgroundImage='url(pic/btn_jj2x.png)'\" onclick='clicktv("+data.datas[5*i+2].pID+");' >"+(5*i+3)+"</td>";
						}
						var id3 = data.datas[5*i+3].pID;
						if(curNumber==id3){
							divStr+="<td background='pic/btn_pre2x.png' width='47px' height='47px' id='title4' align='center' >"+(5*i+4)+"</td>";
						}else{
							divStr+="<td background='pic/btn_jj2x.png' width='47px' height='47px' id='title3' align='center' onmouseover=\"this.style.backgroundImage='url(pic/btn_hover2x.png)'\" onmouseout=\"this.style.backgroundImage='url(pic/btn_jj2x.png)'\" onclick='clicktv("+data.datas[5*i+3].pID+");' >"+(5*i+4)+"</td>";
						}
						var id4 = data.datas[5*i+4].pID;
						if(curNumber==id4){
							divStr+="<td background='pic/btn_pre2x.png' width='47px' height='47px' id='title4' align='center' >"+(5*i+5)+"</td></tr>";
						}else{
							divStr+="<td background='pic/btn_jj2x.png' width='47px' height='47px' id='title3' align='center' onmouseover=\"this.style.backgroundImage='url(pic/btn_hover2x.png)'\" onmouseout=\"this.style.backgroundImage='url(pic/btn_jj2x.png)'\" onclick='clicktv("+data.datas[5*i+4].pID+");' >"+(5*i+5)+"</td></tr>";
						}
					...
					if(tvmore>0){
						...
						for(var j=0;j<tvmore;j++){
							var id0 = data.datas[5*tvint+j].pID;
							if(curNumber==id0){
								divStr+="<td background='pic/btn_pre2x.png' width='47px' height='47px' id='title4' align='center' >"+(5*tvint+j+1)+"</td>";
							}else{
								divStr+="<td background='pic/btn_jj2x.png' width='47px' height='47px' id='title3' align='center' onmouseover=\"this.style.backgroundImage='url(pic/btn_hover2x.png)'\" onmouseout=\"this.style.backgroundImage='url(pic/btn_jj2x.png)'\" onclick='clicktv("+data.datas[5*tvint+j].pID+");' >"+(5*tvint+j+1)+"</td>";
							}					
						}
						divStr+="</tr>";
					}			
				...
				}else if(playType=="VARIETY_SHOW"){//综艺
					initMenu(1,3);
					iszhuiju(2);
					var updateTime = data.updateEP;
					if(updateTime==""){
						updateTime=name;
					}
					...
					var varietySize = data.datas.length;
					var tmpCidlist="";
					...
					for(var m=0;m<varietySize;m++){
						var contid = data.datas[m].pID;
						var newName = data.datas[m].name;
						var isit = data.datas[m].newIsit;
						//var works = newName.substring(0,8);
						var imgH =data.datas[m].pics.highResolutionH;
						if(imgH==""){
								imgH = data.datas[m].pics.highResolutionV;
								if(imgH==""){
						...
						if(varietySize==1){
							...
							newName = name;
							//isit=empInfo[0].isit;
							imgH= data.pics.highResolutionH;
							if(imgH==""){
								imgH = data.pics.highResolutionV;
								if(imgH==""){
									imgH="pic/default2.png";
								}
							}
					...
					var imgH = data.pics.highResolutionH;
					if(imgH==""){
						imgH = data.pics.highResolutionV;
						if(imgH==""){
							imgH="pic/default2.png";
					...
					divStr+="</div></td><td width='10px'></td><td width='118px' height='51px' valign='top' class='movietitle' >"+subString(name,40,true)+"<br />"+area+"&nbsp;"+year+"</td>";
					divStr+="</tr></table>";
				...
				// 修改人 kx 根据接口数据返回中 data.playing.tip.code="USE_TICKET" 提示用券提示 判断放在这儿是否符合要求 问研发
				if (typeof(data.playing.tip) == "undefined"){
				}else if ((typeof(data.playing.tip.code)!="undefined") && ("USE_TICKET" == data.playing.tip.code)){
					migu_app.ShowMiniEPG("networkout.html?id=10",0);
					document.getElementById("frame1").src="http://www.miguvideo.com/mgs/client/comment/prd/clientComment.html?cid="+cids+"&mId="+mId+"&pageIndex=1";
					return
				}
				play();
				firstPlay=1;
...
//拼接节目id信息
function getSubSerial_IDS_List(SubSerial_IDS){
	var SubSerial_ID_Str=""
	for (var index in SubSerial_IDS){
		SubSerial_ID_Str+=SubSerial_IDS[index].pID+",";
	}
	if (SubSerial_ID_Str.length>0){
		SubSerial_ID_Str = SubSerial_ID_Str.substr(0, SubSerial_ID_Str.length - 1);
	}
	return SubSerial_ID_Str
}
//为你推荐
...
function getUserInfos(){
	...
	try{
		$.ajax({
				url:"http://www.miguvideo.com/gateway/user/v1/user-info/",					
				async:true,
				cache:false,
				beforeSend: function (XMLHttpRequest) {
                    XMLHttpRequest.setRequestHeader("userId",userId);
					XMLHttpRequest.setRequestHeader("userToken", userToken);
					XMLHttpRequest.setRequestHeader("SDKCEId", "79acd784-cbbb-4cae-8778-8723e001164b");
                },
				success: function(data, textStatus, jqXHR ){
					if(data.code == "401"){//未登陆
						visitor_type=0;
						user_id="";
						account_name="";
					}else if(data.code == "200"){//已登陆
						visitor_type=1;
						user_id = data.body.data.userId;
						account_name = data.body.data.sname;
					}
				},
...
//上报播放记录 修改为新的上报接口 修改时间 2019-04-17 修改人 kx
		...
		$.ajax({
				...
				success: function(data, textStatus, jqXHR ){
					if(data.code == "401"){//未登陆
						...
						//}
						if(data.body.data.picture){
							migu_app.updateUserInfo(data.body.data.picture);
						}else{
							migu_app.updateUserInfo("1");//设置默认图片
						}
	...
	if(isPingBi==1){
		...
		if(infoString){
			...
			if(info){
				...
				xhr.setRequestHeader("X-UP-CLIENT-CHANNEL-ID", "0125_10010001005");
				xhr.setRequestHeader("channel", "0125");
				xhr.setRequestHeader("clientId",getNewGuid())
			} 
		}else {
			xhr.setRequestHeader("SDKCEId", "79acd784-cbbb-4cae-8778-8723e001164b");
			xhr.setRequestHeader("X-UP-CLIENT-CHANNEL-ID", "0125_10010001005");
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4&&xhr.status==200) {
				...
				if(empInfo.code==409){
					...
					//返回结果中 drm=true 提示 migu_app.ShowMiniEPG("networkout.html?id=11",0);
					if (empInfo.body.content.drm == true) {
						...
						migu_app.SetConfig("needClothHat",""+empInfo.body.urlInfo.needClothHat);
						if(isLogins()){
...
function LoginStatus(type){
	try{
		if(isLogins()){
			...
			//http://www.miguvideo.com/wap/resource/miguPC_client/memberCenter/user_center.jsp");
		}else{
			if(type==0){
...
function payorder(){
	...
	migu_app.ShowMiniEPG("http://www.miguvideo.com/mgs/website/prd/buyVip.html",callbackUrl,800,570);
}
...
function autoHeight(){
    ...
    try{
		if((webH-55)!=$("#scrollbox").height()){
			...
			$("#frame1").height((webH-55) + 'px');//解决互动 评论列表 下拉无效的问题 暂时注掉 2019-04-12 修改人 kuangxing
...
//添加追剧
function zhuiju(){
	//点击追剧 判断是否登录，如果未登录，追剧图片置灰，并弹出登录框  修改人 kx 2019-03-29
	if (!isLogins()){
		//跳转登录页面
	...
	//点击追剧 调用新接口  修改人 kuangxing  2019-04-11
	var infoString = getUserInfo();
	var info;
	if(infoString){
		infoString = infoString.replace(/'/g, "\"");
	...
	try{
		...
		if(iszj==0){
			...
			var hurl = "http://www.miguvideo.com/gateway/uic/v1/user-favorites/?mId="+mId+"&type=1&platform=0005&source=H5&contentId="+cids;
			...
			xhr.open("DELETE",hurl, true);
		}
		xhr.setRequestHeader("userId",userId);
		xhr.setRequestHeader("userToken", userToken);
		xhr.setRequestHeader("channel", "H5");
		xhr.setRequestHeader("clientId", "79acd784-cbbb-4cae-8778-8723e001164b");
		xhr.setRequestHeader("SDKCEId", "79acd784-cbbb-4cae-8778-8723e001164b");
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4&&xhr.status==200) {
				var empInfo = JSON.parse(xhr.responseText);
				if(empInfo.body.resultCode=="0000"){
					if(iszj==0){
						if(zjtype==1){
				...
				}else if(empInfo.body.resultCode=="1014"){
					migu_app.Login("miguplay://html/vodpage.html?playcid="+cids);
				}else if(empInfo.body.resultCode=="1062"){
					migu_app.ShowMiniEPG("msg.html?msg=重复信息!","",140,50);
				}else if(empInfo.body.resultCode=="0057"){
					migu_app.ShowMiniEPG("msg.html?msg=参数异常!","",140,50);
				}else if(empInfo.body.resultCode=="0402"){
					migu_app.ShowMiniEPG("msg.html?msg=数据查询失败!","",140,50);
				}else if(empInfo.body.resultCode=="0007"){
					migu_app.ShowMiniEPG("msg.html?msg=未知异常!","",140,50);
				}else if(empInfo.body.resultCode=="0004"){
					migu_app.ShowMiniEPG("msg.html?msg=数据库异常!","",140,50);
				}else if(empInfo.body.resultCode=="0012"){
...
function iszhuiju(type){
	try{
		...
		zjtype = type;
		var xhr = getXHR();
		//判断是否收藏 调用新接口  修改人 kuangxing  2019-04-11
		//var hurl = "http://www.miguvideo.com/uic-service/favroites/isCollected?userId="+user_id+"&mIds="+mId+"&type=1&platform=0005";
		var hurl = "http://www.miguvideo.com/gateway/uic/v1/user-favorites/"+mId+"?type=1&mId="+mId+"&platform=0005";
		xhr.open("GET",hurl, true);
		xhr.setRequestHeader("userId",userId);
		xhr.setRequestHeader("userToken", userToken);
		xhr.setRequestHeader("channel", "H5");
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4&&xhr.status==200) {
				...
				if(type==1){
					...
					if(empInfo.body.resultCode=="0000"){
						var strJson = JSON.stringify(empInfo.body.resultList);
		...
		$.ajax({
				...
				success: function(data, textStatus, jqXHR ){
					...
					if(data.code == "401"){//未登陆
						...
						migu_app.Login("http://www.miguvideo.com/mgs/website/prd/personalCenter.html#/history?plt=clt");
						//http://www.miguvideo.com/wap/resource/miguPC_client/memberCenter/my_record.jsp");
					}else if(data.code == "200"){//已登陆
						visitor_type=1;
						user_id = data.body.data.userId;
						account_name = data.body.data.sname;
		...
		$.ajax({
				...
				success: function(data, textStatus, jqXHR ){
					if(data.resultCode == "1014"){//未登陆
						...
						migu_app.updateUserInfo("");
						//migu_app.Login("http://www.miguvideo.com/wap/resource/miguPC_client/memberCenter/my_record.jsp");
						migu_app.Login("http://www.miguvideo.com/mgs/website/prd/personalCenter.html#/history?plt=clt");//http://www.miguvideo.com/wap/resource/miguPC_client/memberCenter/my_record.jsp");
					}else if(data.resultCode == "0000"){//已登陆
						visitor_type=1;
						user_id=data.data.userId;
						account_name=data.data.name;
